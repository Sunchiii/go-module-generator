...
func CreatePagination(projectName string) {
	...
	if os.IsNotExist(err) {
		...
		fmt.Fprintf(destination, "}\n\n")
		fmt.Fprintf(destination, "func Paginate(db *gorm.DB, model interface{}, paginate PaginateRequest, results interface{}) (*PaginatedResponse, error) {\n\n")
		fmt.Fprintf(destination, "	if paginate.Limit <= 0 {\n")
		fmt.Fprintf(destination, "		return nil, errors.New(\"limit must be greater than 0\")\n")
		fmt.Fprintf(destination, "	}\n")
		...
		fmt.Fprintf(destination, "	// Fetch paginated results\n")
		fmt.Fprintf(destination, "	result := db.Preload(clause.Associations).Limit(paginate.Limit).Offset(offset).Find(results)\n")
		...
		fmt.Fprintf(destination, "		Rows:         results,\n")
